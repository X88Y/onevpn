...
MVMVpn is a cross-platform VPN client application built with Flutter. It wraps the Xray-core engine (via the sibling `libMVM` project) and provides a unified UI for Android, iOS, macOS, Windows, and Linux.
...
| Native iOS/macOS | Swift + `LibMVM.xcframework` |
...
│   ├── All/                      # LibMVM.xcframework + bridge files
...
├── c/include/                    # C header for ffigen (`libMVM.h`)
...
- The app requires **pre-built `libMVM` artifacts** for every target platform. These are built from the sibling `libMVM` repository and copied into platform-specific directories (see `readme/FIRST_RUN.md` for exact copy commands).
...
- **Desktop FFI**: Linux and Windows talk to `libMVM` via `dart:ffi`. Bindings are generated by `ffigen` from `c/include/libMVM.h`.
...
- **Desktop binaries** (`MVMVpnCore`, `libMVM.so`, `libMVM.dll`) are built externally and copied in; do not commit prebuilt native libraries.
...
| Add a new FFI binding | Update `c/include/libMVM.h`, then run `ffigen` |
...
- Desktop builds (Linux/Windows) require `libMVM` artifacts and a helper binary named `MVMVpnCore` (the Xray-core executable). See `readme/FIRST_RUN.md` for exact copy instructions.
