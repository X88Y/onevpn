...
REQUESTS_PER_SECOND = 51   # VK разрешает до 3 запросов/сек
...
def delete_posts(vk, owner_id: int, post_ids: list[int]) -> None:
    """Удалить все переданные посты с использованием 2 потоков."""
    total = len(post_ids)
    print(f"\nНачинаем удаление {total} постов в 2 потока...")

    def delete_single_post(i, post_id):
        retries = 5
        while retries > 0:
            try:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = [
            executor.submit(delete_single_post, i, post_id)
